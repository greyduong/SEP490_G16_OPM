        ...
        <div class="px-5">
            <div class="flex relative">
                ...
                <div class="sticky top-0 rounded-sm border border-slate-300 p-3 w-65 flex flex-col gap-5">
                    <!-- Sort -->
                    <form class="flex flex-col gap-2" method="get" action="home">
                    ...
                    <!-- Nút xóa bộ lọc -->
                    <c:if test="${not empty param.keyword or not empty param.categoryName or not empty param.sort}">
                        <a href="home" class="btn btn-sm btn-outline-secondary">Xóa bộ lọc</a>
                    </c:if>
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="*:!text-slate-600 flex flex-col gap-2">
                            <a href="home?keyword=${param.keyword}&sort=${param.sort}">Tất cả</a>
                            <c:forEach var="c" items="${categoryList}">
                                <a href="home?categoryName=${c.name}&keyword=${param.keyword}&sort=${param.sort}">${c.name}</a>
                            </c:forEach>
                        </div>
                    </div>
                </div>
                        ...
                        <c:forEach var="o" items="${offerList}">
                            <div class="border border-slate-300 flex flex-col rounded-sm overflow-hidden">
                                <div class="relative" style="position: relative;">
                                    ...
                                    <!-- Thêm thẻ a phủ toàn bộ ảnh để nhấn vào ảnh dẫn đến chi tiết -->
                                    <a href="PigsOfferDetails?offerId=${o.offerID}" 
                                       style="position: absolute;top:0;left:0;width:100%;height:100%;z-index:1;display:block;"></a>
                                    <a class="absolute block !text-slate-200 !text-shadow-sm !text-shadow-slate-600 bottom-0 p-2 right-0 left-0 text-lg truncate font-bold !no-underline z-10" 
                                       href="PigsOfferDetails?offerId=${o.offerID}">${o.name}</a>
                                </div>
                                <div class="p-3 flex flex-col gap-3 grow-1">
                                    ...
                                    <div class="mt-auto flex flex-col gap-2">
                                        ...
                                        <div>
                                            <a href="#" class="open-cart-modal inline-flex items-center justify-center !bg-lime-600 hover:!bg-lime-700 text-white py-1 px-2 rounded-sm text-sm gap-2 !no-underline"
                                               data-offer-id="${o.offerID}" data-max="${o.quantity}" data-min="${o.minQuantity}">
                            ...
                            <ul class="pagination justify-content-center">
                                <c:forEach var="i" begin="1" end="${totalPages}">
                                    <li class="page-item ${i == currentPage ? 'active' : ''}">
                                        <a class="page-link" href="home?page=${i}&categoryName=${param.categoryName}&keyword=${param.keyword}&sort=${param.sort}">
                                            ${i}
                                        </a>
                                    </li>
                                </c:forEach>
